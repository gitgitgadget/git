...
test_expect_success '"matching" fails if none match' '
	...
	needle="Perhaps you should specify a branch such as '\''main'\''" &&
...
test_pushmain_workflow success current main

# update parent1's foo (which is our upstream)
test_pushdefault_workflow success upstream foo

# upstream is foo which is not the name of the current branch
test_pushmain_workflow failure simple main

# main and foo are updated
test_pushmain_workflow success matching main

# main is updated
test_pushmain_workflow success current main triangular
...
# with the same name.
test_pushmain_workflow success simple main triangular

# main is updated (parent2 does not have foo)
test_pushmain_workflow success matching main triangular
