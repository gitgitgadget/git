...
prepare_dest () {
	...
	git --git-dir=dest.git config core.bigFileThreshold "$1"
}
...
test_expect_success 'unpack-objects failed under memory limitation' '
	prepare_dest 2m &&
	test_must_fail git --git-dir=dest.git unpack-objects <pack-$PACK.pack 2>err &&
	grep "fatal: attempting to allocate" err
'

test_expect_success 'unpack-objects works with memory limitation in dry-run mode' '
	prepare_dest 2m &&
	git --git-dir=dest.git unpack-objects -n <pack-$PACK.pack &&
...
test_expect_success 'unpack big object in stream' '
	prepare_dest 1m &&
	git --git-dir=dest.git unpack-objects <pack-$PACK.pack &&
	test_dir_is_empty dest.git/objects/pack
'
...
test_expect_success 'unpack big object in stream (core.fsyncmethod=batch)' '
	...
	GIT_TEST_FSYNC=true \
		git --git-dir=dest.git $BATCH_CONFIGURATION unpack-objects <pack-$PACK.pack &&
	...
	test_dir_is_empty dest.git/objects/pack &&
	git --git-dir=dest.git cat-file --batch-check="%(objectname)" <obj-list >current &&
	cmp obj-list current
'

test_expect_success 'do not unpack existing large objects' '
	prepare_dest 1m &&
	git --git-dir=dest.git index-pack --stdin <pack-$PACK.pack &&
	git --git-dir=dest.git unpack-objects <pack-$PACK.pack &&
