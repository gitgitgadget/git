 ...
 *
 *	commit NAME BRANCH SUBJECT [parent=PARENT]... [PATH:BLOB]...
 *	    Creates a commit on refs/heads/BRANCH using the listed
 *	    `PATH:BLOB` mappings as the entire tree (no inheritance
 *	    from parents). Zero or more `parent=` arguments may be
 *	    given in order: the first becomes fast-import's `from`,
 *	    each subsequent one becomes a `merge` parent. Without any
 *	    `parent=` the commit defaults to the current branch tip,
 *	    or becomes a root if BRANCH has no tip yet.
 *
			...
			int argc, const char **argv, int seq)
{
	 ...
	 * operations; emit them first regardless of argv ordering. The
	 * first `parent=` becomes `from`, subsequent ones become
	 * `merge`, in the order given.
	 */
	for (i = 4; i < argc; i++) {
		if (!skip_prefix(argv[i], "parent=", &rest))
			continue;
		fprintf(out, "%s :%d\n",
			seen_parent ? "merge" : "from",
	...
	for (i = 4; i < argc; i++) {
		...
		colon = strchr(argv[i], ':');
		if (!colon)
			die("bad commit spec '%s' (need PATH:BLOB or parent=NAME)",
			    argv[i]);
		key_len = colon - argv[i];
		path = xmemdupz(argv[i], key_len);
		fprintf(out, "M 100644 :%d %s\n",
			resolve_mark(names, colon + 1), path);
		free(path);
	}
