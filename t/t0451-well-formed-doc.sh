...
test_expect_success 'delimited sections have an empty line before when needed' '
	for f in $(cd $GIT_BUILD_DIR/ && git ls-files '\''Documentation/**.adoc'\'')
	do
		awk "
...
" $GIT_BUILD_DIR/$f || (echo "awk failed for $f"; return 1)
	done > out 2> err &&
	test_must_be_empty out
'
...
test_expect_success 'no synopsis lego of option names in definition list items' '
