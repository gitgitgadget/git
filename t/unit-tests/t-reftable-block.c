...
static void test_ref_block_read_write(void)
{
...
static void test_log_block_read_write(void)
{
	...
	for (i = 0; i < N; i++) {
		...
		rec.u.log.refname = xstrdup(name);
...
static void test_obj_block_read_write(void)
{
	const int header_off = 21;
	struct reftable_record recs[30];
	const size_t N = ARRAY_SIZE(recs);
...
static void test_index_block_read_write(void)
{
	const int header_off = 21;
	struct reftable_record recs[30];
	const size_t N = ARRAY_SIZE(recs);
...
{
	TEST(test_ref_block_read_write(), "read-write operations on ref blocks work");
	TEST(test_log_block_read_write(), "read-write operations on log blocks work");
	TEST(test_obj_block_read_write(), "read-write operations on obj blocks work");
	TEST(test_index_block_read_write(), "read-write operations on index blocks work");
