...
test_expect_success 'setup' '
	...
	# push-time INFINITY-gate in paint_down_to_common.  With both tips
	# outside the graph, their generation is INFINITY and the priority
	# queue falls back to commit-date order, which here is non-monotonic
	# between parent and child.  If the optimization were not disabled,
	# a cross-side flag transition on an already-dequeued INFINITY parent
	# would underflow the exclusive counts.
	#
