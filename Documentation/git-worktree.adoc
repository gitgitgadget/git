...
`--recurse-submodules`::
	After the new worktree has been checked out, initialize and update
	each submodule in it, as if running `git submodule update --init
	--recursive` from within the new working tree.  Each submodule's
	gitdir is set up as a linked worktree of the shared submodule
	repository at `$GIT_DIR/modules/<name>/`, so that the submodule's
	objects and refs are shared across all worktrees.
+
Incompatible with `--no-checkout` and `--orphan`.
