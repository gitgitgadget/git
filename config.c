				  ...
				  void *data, enum config_scope scope)
{
	...
	int access_success_count = 0;
	int nonzero_ret_on_global_access_error = scope == CONFIG_SCOPE_GLOBAL;
	char *xdg_config = NULL;
	...
	if (!opts->ignore_global) {
		...
		if (xdg_config) {
			if (!access_or_die(xdg_config, R_OK, ACCESS_EACCES_OK)) {
				ret += git_config_from_file_with_options(
					fn, xdg_config, data,
					CONFIG_SCOPE_GLOBAL, NULL);
				if (nonzero_ret_on_global_access_error && !ret) {
		...
		if (user_config) {
			if (!access_or_die(user_config, R_OK, ACCESS_EACCES_OK)) {
				ret += git_config_from_file_with_options(
					fn, user_config, data,
					CONFIG_SCOPE_GLOBAL, NULL);
				if (nonzero_ret_on_global_access_error && !ret) {
		...
		if (nonzero_ret_on_global_access_error && !access_success_count) {
			--ret;
		}
