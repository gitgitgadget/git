...
void validate_no_verify(struct repository *r, const char *opt)
{
	...
	if (!r || (repo_config_get_value(r, "hook.allownoverify", &val) &&
		   repo_config_get_value(r, "hooks.allownoverify", &val)))
		return;
	...
	if (maybe_bool == 1) {
		...
		warning(_("bypassing hooks with '%s' is discouraged by 'hook.allowNoVerify'"), opt);
		return;
		...
		advise(_("this repository disallows '%s' as a workflow guardrail against accidental bypass.\n"
			 ...
			 "  git -c hook.allowNoVerify=true <command>\n"
			 "or:\n"
			 "  GIT_ALLOW_NO_VERIFY=1 git <command>"), opt);
		die(_("the use of '%s' is disabled by 'hook.allowNoVerify'"), opt);
	} else {
		warning(_("unknown value for 'hook.allowNoVerify': '%s'"), val);
	}
}
