	...
	while ((de = readdir_skip_dot_and_dotdot(dir)) != NULL) {
		...
		if (S_ISDIR(fs.st_mode)) {
			if (add_watch(current.buf, data))
				goto failed;
			if (register_inotify(current.buf, data, batch))
				goto failed;
		} else if (batch) {
			fsmonitor_batch__add_path(batch, current.buf);	
		}
	}
	ret = 0;
...
int fsm_listen__ctor(struct fsmonitor_daemon_state *state)
{
	...
	if (add_watch(state->path_worktree_watch.buf, data))
		ret = -1;
	else if (register_inotify(state->path_worktree_watch.buf, data, NULL))
		ret = -1;
	else if (state->nr_paths_watching > 1) {
		if (add_watch(state->path_gitdir_watch.buf, data))
			ret = -1;
		else if (register_inotify(state->path_gitdir_watch.buf, data, NULL))
			ret = -1;
	}
	...
	switch (fsmonitor_classify_path_absolute(state, path)) {
		...
		case IS_WORKDIR_PATH:
			...
			if (em_dir_created(event->mask)) {
				...
				if (register_inotify(path, state->listen_data, batch)) {
					state->listen_data->shutdown = SHUTDOWN_ERROR;
					goto done;
				}
			}
